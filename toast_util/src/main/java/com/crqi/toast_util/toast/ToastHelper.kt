 ...
 * desc   : 代理Toast 具体实现类
 */
internal class ToastHelper(application: Application) : Runnable {

    // WindowManager 处理消息线程
    private val mHandler = Handler(Looper.getMainLooper())

    // WindowManager 辅助类
    private val mWindowHelper: WindowHelper = WindowHelper(this, application)
    ...
    private var curToast: IToastStyle? = null

    /***
     * 显示吐司弹窗
     */
    fun show(curToast: IToastStyle?) {
        if (this.curToast != null) {
            cancel()
        }
        this.curToast = curToast
        if (this.curToast?.getToastView() != null) {
            /*
            ...
            */
            ...
            params.title = Toast::class.java.simpleName
            params.flags = (WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON
                    or WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
                    or WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
            params.packageName = mPackageName
            // 重新初始化位置
            params.gravity = curToast!!.getGravity()
            params.x = curToast.getXOffset()
            params.y = curToast.getYOffset()
            ...
            try {
                ...
                curToast.getToastText()?.apply {
                    curToast.getToastView()?.setText(text = this, config = curToast)
                }
                curToast.getToastView()?.setBackGroundDrawable(config = curToast)
                mWindowHelper.windowManager.addView(curToast.getToastView(), params)
                // 当前已经显示
                isShow = true
                // 添加一个移除吐司的任务
                mHandler.postDelayed(this, curToast.getDuration().toLong())
    ...
    fun cancel() {
        ...
        if (curToast?.getToastView() != null) {
            try {
                ...
                mWindowHelper.windowManager.removeView(curToast?.getToastView())
            } catch (ignored: NullPointerException) {
                ignored.printStackTrace()
